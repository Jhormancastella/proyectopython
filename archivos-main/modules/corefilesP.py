...
def NewFile(data):
    try:
        with open(MY_DATABASEP, "w") as wf:
...
def UpdateFile(data):
    try:
        with open(MY_DATABASEP, 'w') as fw:
...
def AddData(*param):
    try:
        data = list(param)
        with open(MY_DATABASEP, "r+") as rwf:
...
def ReadFile():
    try:
        with open(MY_DATABASEP, "r") as rf:
            return json.load(rf)
    except (IOError, json.JSONDecodeError) as e:
...
def checkFile(data=None):
    try:
        if os.path.isfile(MY_DATABASEP):
            if data is not None:
